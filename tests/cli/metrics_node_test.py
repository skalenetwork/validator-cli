...
def test_metrics(runner):
    ...
    print(output_list)
    assert '       Date           Bounty   Downtime   Latency' == output_list[-6]
    assert '-------------------------------------------------' == output_list[-5]

    assert f'{metrics[0][0]}    {metrics[0][1]:.1f}          {metrics[0][2]}       {metrics[0][3]:.1f}' == output_list[-4]  # noqa
...
def test_metrics_limited(runner):
    ...
    output_list = result.output.splitlines()[-row_count:]
    print(output_list)
    assert '       Date           Bounty   Downtime   Latency' == output_list[-5]
    assert '-------------------------------------------------' == output_list[-4]
    assert f'{metrics[0][0]}    {metrics[0][1]:.1f}          {metrics[0][2]}       {metrics[0][3]:.1f}' == output_list[-3]  # noqa
    assert '' == output_list[-2]
    assert f' Total bounty per the given period: {total_bounty:.3f} SKL' == output_list[-1]  # noqa


def test_metrics_since_not_empty(runner):
...
def test_metrics_since_empty(runner):
    ...
    output_list = result.output.splitlines()[-row_count:]
    print(output_list)
    assert 'Date   Bounty   Downtime   Latency' == output_list[-4]
    assert '----------------------------------' == output_list[-3]
    assert '' == output_list[-2]
    assert f' Total bounty per the given period: {total_bounty:.3f} SKL' == output_list[-1]  # noqa


def test_metrics_till_not_empty(runner):
    end_date = '2100-01-01'
    metrics, total_bounty = get_metrics_from_events([NODE_ID], limit=1,
