...
from utils.web3_utils import (
    init_skale_from_config, init_skale_w_wallet_from_config)
...
def accept_all_delegations(pk_file: str, fee: Optional[TxFee]) -> None:
    skale = init_skale_w_wallet_from_config(pk_file)
    if not skale:
        return
    fee = fee or TxFee(gas_price=skale.gas_price)
...
def link_node_address(node_address: str,
                      signature: str,
                      pk_file: str,
...
def unlink_node_address(node_address: str, pk_file: str, fee: Optional[TxFee]) -> None:
    skale = init_skale_w_wallet_from_config(pk_file)
    if not skale:
        return
    fee = fee or TxFee(gas_price=skale.gas_price)
...
def withdraw_fee(recipient_address: str, pk_file: str, fee: Optional[TxFee]) -> None:
    skale = init_skale_w_wallet_from_config(pk_file)
    if not skale:
        return
    fee = fee or TxFee(gas_price=skale.gas_price)
...
def set_mda(new_mda: int, pk_file: str, fee: Optional[TxFee]) -> None:
    skale = init_skale_w_wallet_from_config(pk_file)
    if not skale:
        return
    fee = fee or TxFee(gas_price=skale.gas_price)
    with yaspin(text='Changing minimum delegation amount', color=SPIN_COLOR) as sp:
        ...
        sp.write(
            f'✔ Minimum delegation amount for your validator ID changed to {new_mda}')
        print(f'Transaction hash: {tx_res.tx_hash}')


def change_address(address: str, pk_file: str, fee: Optional[TxFee]) -> None:
    skale = init_skale_w_wallet_from_config(pk_file)
    if not skale:
        return
    fee = fee or TxFee(gas_price=skale.gas_price)
...
def confirm_address(validator_id: int, pk_file: str, fee: Optional[TxFee]) -> None:
    skale = init_skale_w_wallet_from_config(pk_file)
    if not skale:
        return
    fee = fee or TxFee(gas_price=skale.gas_price)
...
def edit(name: str, description: str, pk_file: str, fee: Optional[TxFee]) -> None:
    skale = init_skale_w_wallet_from_config(pk_file)
    if not skale:
        return
    ...
    try:
        ...
        print(
            f'Validator ID is not found for your address: {skale.wallet.address}')
        sys.exit(2)

    validator = skale.validator_service.get_with_id(validator_id)

    if name:
        change_validator_name(name, skale, validator, fee)
    if description:
        change_validator_description(description, skale, validator, fee)


def change_validator_name(name, skale, validator, fee):
...
def change_validator_description(description, skale, validator, fee):
